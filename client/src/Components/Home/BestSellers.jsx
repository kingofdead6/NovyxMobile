...
function TiltCard({ product, index, onClick }) {
  ...
  return (
    ...
    >
      ...
      {/* Phone visual */}
      <div style={{ position: "relative", height: 248, display: "flex", alignItems: "center", justifyContent: "center", background: "radial-gradient(circle at 50% 30%,rgba(139,92,246,.18),transparent 60%)", overflow: "hidden" }}>
        {product.badge && (
          <span style={{ position: "absolute", top: 13, left: 13, zIndex: 4, fontFamily: "'JetBrains Mono'", fontSize: 10.5, fontWeight: 600, padding: "5px 10px", borderRadius: 8, background: "rgba(34,211,238,.15)", border: "1px solid rgba(34,211,238,.4)", color: "#22D3EE" }}>
            {product.badge}
          </span>
        )}
      ...
      <div style={{ position: "relative", zIndex: 4, padding: 18 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <span style={{ width: 36, height: 36, borderRadius: 11, background: "linear-gradient(135deg,#8B5CF6,#6C2BD9)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, boxShadow: "0 8px 20px -6px rgba(108,43,217,.8)" }}>→</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <section style={{ maxWidth: 1280, margin: "0 auto", padding: "54px 26px 30px" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, marginBottom: 30, flexWrap: "wrap" }}>
        <div>
          <p style={{ fontFamily: "'JetBrains Mono'", fontSize: 12.5, letterSpacing: ".12em", color: "#22D3EE", margin: "0 0 10px" }}>// TRENDING NOW</p>

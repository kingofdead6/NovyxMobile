...
export default function AdminCases() {
  ...
  const [form, setForm] = useState({ name: "", brand: "", price: "", stock: "", material: "", color: "", description: "", compatibleModels: "", images: [] });
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/cases`, { headers: authH() });
      setItems(res.data);
    } catch {
  ...
  useEffect(() => {
    setFiltered(items.filter(p => {
      ...
      const matchBrand = !selectedBrand || p.brand?._id === selectedBrand || p.brand === selectedBrand;
      return matchSearch && matchBrand;
    }));
  }, [items, searchTerm, selectedBrand]);

  const resetForm = () => {
    setForm({ name: "", brand: "", price: "", stock: "", material: "", color: "", description: "", compatibleModels: "", images: [] });
    setEditingId(null);
    setShowModal(false);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    ["name", "brand", "price", "stock", "material", "color", "description"].forEach(k => fd.append(k, form[k]));
    ...
    try {
      if (editingId) {
        await axios.put(`${API_BASE_URL}/cases/${editingId}`, fd, { headers: authH() });
        toast.success("Case updated");
      } else {
        await axios.post(`${API_BASE_URL}/cases`, fd, { headers: authH() });
  ...
  const handleEdit = (item) => {
    ...
    setForm({ name: item.name, brand: item.brand?._id || item.brand || "", price: item.price, stock: item.stock ?? "", material: item.material || "", color: item.color || "", description: item.description || "", compatibleModels: (item.compatibleModels || []).join(", "), images: item.images.map(i => ({ preview: i.url, url: i.url })) });
    setShowModal(true);
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/cases/${id}`, { headers: authH() });
  ...
  return (
    <div className="min-h-screen pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        <h1 className="text-4xl sm:text-5xl font-light mb-8">Phone Cases</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <button onClick={() => setShowModal(true)} className="bg-black text-white px-6 py-4 rounded-2xl flex items-center justify-center gap-3 hover:bg-gray-800 transition w-full sm:w-auto">
            <Plus size={22} /> New Case
          </button>
          <div className="flex-1 flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
              <input type="text" placeholder="Search cases..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-11 py-4 border border-gray-300 rounded-2xl focus:border-black outline-none" />
            </div>
            <select value={selectedBrand} onChange={e => setSelectedBrand(e.target.value)} className="w-full sm:w-52 py-4 px-4 border border-gray-300 rounded-2xl focus:border-black outline-none">
        ...
        {filtered.length === 0 && !loading && <p className="text-center text-gray-500 py-20">No cases found</p>}
      </div>
          ...
          <div className="fixed inset-0 bg-black/70 z-50 flex justify-center p-4">
            <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }}
              ...
              <div className="p-6 pb-4 flex justify-between items-center border-b">
                <h2 className="text-xl font-semibold">{editingId ? "Edit Case" : "New Case"}</h2>
              ...
              <div className="overflow-y-auto px-6 py-5">
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <select required value={form.brand} onChange={e => setForm({ ...form, brand: e.target.value })} className="w-full p-4 border rounded-2xl">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <input placeholder="Material" value={form.material} onChange={e => setForm({ ...form, material: e.target.value })} className="p-4 border rounded-2xl" />
                    <input placeholder="Color" value={form.color} onChange={e => setForm({ ...form, color: e.target.value })} className="p-4 border rounded-2xl" />
                  </div>
                  <input placeholder="Compatible models (comma-separated)" value={form.compatibleModels} onChange={e => setForm({ ...form, compatibleModels: e.target.value })} className="w-full p-4 border rounded-2xl" />

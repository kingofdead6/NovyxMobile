...
const STATUS_COLORS = {
  ...
  in_delivery: "background:rgba(139,92,246,.15);color:#c4b5fd;border:1px solid rgba(139,92,246,.3)",
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 32, justifyContent: "center" }}>
          {["all", "pending", "confirmed", "in_delivery", "reached", "canceled"].map(s => (
            <button
              ...
              style={{ padding: "8px 18px", borderRadius: 999, border: `1px solid ${selectedStatus === s ? "rgba(139,92,246,.5)" : "rgba(255,255,255,.1)"}`, background: selectedStatus === s ? "rgba(139,92,246,.2)" : "rgba(255,255,255,.04)", color: selectedStatus === s ? "#c4b5fd" : "#94A3B8", fontFamily: "'JetBrains Mono'", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all .2s", textTransform: "uppercase" }}
            >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 24 }}>
            {filtered.map(order => {
              ...
              return (
                ...
                >
                  {/* Card header */}
                  <div style={{ background: "rgba(139,92,246,.12)", borderBottom: "1px solid rgba(255,255,255,.06)", padding: "20px 20px 16px" }}>
                    ...
                    {order.status === "canceled" ? (
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                        <button
                          ...
                          disabled={!nextStatus}
                          style={{ padding: "11px", borderRadius: 12, border: "none", background: nextStatus ? "linear-gradient(135deg,#8B5CF6,#6C2BD9)" : "rgba(255,255,255,.08)", color: nextStatus ? "#fff" : "#64748B", fontWeight: 700, fontSize: 14, cursor: nextStatus ? "pointer" : "not-allowed" }}
                        >
              ...
              <div style={{ padding: 32 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 24, marginBottom: 28 }}>
                  <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 16, padding: 20 }}>
                    <h3 style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 16, color: "#8B5CF6", margin: "0 0 14px", textTransform: "uppercase", letterSpacing: ".04em" }}>Customer</h3>
                  ...
                  <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 16, padding: 20 }}>
                    <h3 style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 16, color: "#8B5CF6", margin: "0 0 14px", textTransform: "uppercase", letterSpacing: ".04em" }}>Pricing</h3>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 14, color: "#94A3B8" }}>
                  ...
                  {selectedOrder.items.map((item, idx) => (
                    <div key={idx} style={{ display: "flex", gap: 16, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 16, padding: 16, alignItems: "center" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 17, color: "#8B5CF6", margin: 0 }}>{item.price * item.quantity} DA</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {selectedOrder.status !== "canceled" && selectedOrder.status !== "reached" && (
                  <div style={{ display: "flex", gap: 14, paddingTop: 20, borderTop: "1px solid rgba(255,255,255,.08)" }}>
                    <button
                      ...
                      style={{ flex: 1, padding: "14px", background: "linear-gradient(135deg,#8B5CF6,#6C2BD9)", color: "#fff", fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 16, borderRadius: 14, border: "none", cursor: "pointer" }}
                    >

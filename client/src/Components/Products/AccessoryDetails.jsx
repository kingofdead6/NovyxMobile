      ...
      <div style={{ display: "flex", gap: 46, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "0 0 500px", minWidth: 300, maxWidth: 560, position: "sticky", top: 96, animation: "fadeUp .6s both" }}>
          ...
          >
            ...
            {images.length > 0 ? (
              ...
              <div style={{ position: "relative", zIndex: 2, width: 220, height: 220, borderRadius: 36, background: "linear-gradient(165deg,#8B5CF6,#6C2BD9)", transform: "translateZ(50px)" }} />
            )}
          </div>
          {images.length > 1 && (
        ...
        <div style={{ flex: 1, minWidth: 300, animation: "fadeUp .6s both .08s" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
            {item.category?.name && (
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 12, padding: "5px 11px", borderRadius: 8, background: "rgba(139,92,246,.16)", border: "1px solid rgba(139,92,246,.35)", color: "#d7c9ff" }}>{item.category.name}</span>
            )}
            {item.brand && (
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 12, padding: "5px 11px", borderRadius: 8, background: "rgba(34,211,238,.1)", border: "1px solid rgba(34,211,238,.3)", color: "#67e8f9" }}>{item.brand}</span>
            )}
      ...
      {related.length > 0 && (
        <section style={{ marginTop: 64 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 18 }}>
            {related.map(a => <RelatedCard key={a._id} item={a} onClick={() => navigate(`/accessory/${a._id}`)} />)}
          </div>
        </section>
      )}
    </div>

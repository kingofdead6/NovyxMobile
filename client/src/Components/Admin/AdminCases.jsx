...
const S = {
  ...
  brandLabel: { fontSize: 11, color: "#6D28D9", fontFamily: "'JetBrains Mono'", margin: "0 0 4px", fontWeight: 600 },
  cardName: { fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 15, color: "#E2E8F0", margin: 0, lineHeight: 1.3 },
  badge: { display: "inline-block", padding: "2px 9px", borderRadius: 20, fontSize: 11, fontFamily: "'JetBrains Mono'", fontWeight: 600, background: "rgba(34,211,238,.12)", color: "#22D3EE", border: "1px solid rgba(34,211,238,.25)", marginTop: 5 },
...
export default function AdminCases() {
  ...
  const fetchBrands = async () => {
    try { const res = await axios.get(`${API_BASE_URL}/phone-brands`, { headers: authH() }); setBrands(res.data); } catch { /* silent */ }
  };
  ...
  useEffect(() => {
    setFiltered(items.filter(p => {
      ...
      const mb = !selectedBrand || p.brand?._id === selectedBrand || p.brand === selectedBrand;
      return ms && mb;
    }));
  }, [items, searchTerm, selectedBrand]);

  const resetForm = () => { setForm({ name: "", brand: "", price: "", stock: "", material: "", color: "", description: "", compatibleModels: "", images: [] }); setEditingId(null); setShowModal(false); };
          ...
          {filtered.map((item, i) => (
            <motion.div key={item._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div style={S.cardBody}>
                <p style={S.brandLabel}>{item.brand?.name?.toUpperCase() || ""}</p>
                <p style={S.cardName}>{item.name}</p>
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                  {item.color && <span style={S.badge}>{item.color}</span>}
                  {item.material && <span style={{ ...S.badge, background: "rgba(167,139,250,.12)", color: "#A78BFA", border: "1px solid rgba(167,139,250,.25)" }}>{item.material}</span>}
                </div>

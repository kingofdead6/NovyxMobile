...
const ACCORDIONS = [
  { title: "What's included", body: "Accessory item as pictured. Packaging may vary. Check product description for bundled items." },
  { title: "Warranty & returns", body: "6-month warranty on all accessories. 7-day return policy for unopened items in original condition." },
  ...
  return (
    ...
    >
      ...
      <div style={{ height: 200, display: "flex", alignItems: "center", justifyContent: "center", background: "radial-gradient(circle at 50% 30%,rgba(139,92,246,.14),transparent 60%)" }}>
        {item.images?.[0]?.url
          ? <img src={item.images[0].url} alt={item.name} style={{ maxWidth: 120, maxHeight: 170, objectFit: "contain", filter: "drop-shadow(0 20px 30px rgba(0,0,0,.6))" }} />
          : <div style={{ width: 100, height: 100, borderRadius: 20, background: "linear-gradient(165deg,#8B5CF6,#6C2BD9)" }} />
        }
      </div>
      <div style={{ position: "relative", zIndex: 4, padding: 16 }}>
        ...
        <p style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 15, color: "#22D3EE", margin: 0 }}>{(item.price || 0).toLocaleString()} DA</p>
      </div>
    </div>
  );
}
...
export default function AccessoryDetailsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    setSelectedImg(0);
    axios.get(`${API_BASE_URL}/accessories/${id}`)
      .then(r => {
        setItem(r.data);
      ...
      .catch(() => toast.error("Accessory not found"))
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ fontFamily: "'Space Grotesk'", fontSize: 22, color: "#8B5CF6", animation: "glowPulse 1.5s infinite" }}>Loading accessory…</p>
    </div>
  );

  if (!item) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ fontFamily: "'Space Grotesk'", fontSize: 22, color: "#94A3B8" }}>Accessory not found.</p>
    </div>
  );
      ...
      <div style={{ display: "flex", gap: 46, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 300, position: "sticky", top: 96, animation: "fadeUp .6s both" }}>
          ...
          >
            ...
            <div style={{ position: "absolute", width: 280, height: 280, borderRadius: "50%", background: "radial-gradient(circle,rgba(139,92,246,.5),transparent 65%)", filter: "blur(40px)", opacity: .5 }} />
            {images.length > 0 ? (
              <img
                src={images[selectedImg]?.url}
                alt={item.name}
                style={{ position: "relative", zIndex: 2, maxWidth: 320, maxHeight: 400, objectFit: "contain", transform: "translateZ(50px)", filter: "drop-shadow(0 40px 70px rgba(0,0,0,.8))", transition: "transform .4s" }}
              />
            ) : (
              <div style={{ position: "relative", zIndex: 2, width: 200, height: 200, borderRadius: 30, background: "linear-gradient(165deg,#8B5CF6,#6C2BD9)", transform: "translateZ(50px)" }} />
            )}
          </div>
            ...
            <div style={{ display: "flex", gap: 11, marginTop: 16, justifyContent: "center", flexWrap: "wrap" }}>
              {images.map((img, i) => (
                <button
                  ...
                  style={{ width: 66, height: 66, borderRadius: 14, cursor: "pointer", overflow: "hidden", border: `2px solid ${i === selectedImg ? "#8B5CF6" : "rgba(255,255,255,.1)"}`, transition: "all .25s", padding: 0 }}
                >
        ...
        <div style={{ flex: 1, minWidth: 300, animation: "fadeUp .6s both .08s" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
            {item.category?.name && (
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 12, padding: "5px 11px", borderRadius: 8, background: "rgba(139,92,246,.16)", border: "1px solid rgba(139,92,246,.35)", color: "#d7c9ff" }}>
                {item.category.name}
              </span>
            )}
            {item.brand && (
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 12, padding: "5px 11px", borderRadius: 8, background: "rgba(34,211,238,.1)", border: "1px solid rgba(34,211,238,.3)", color: "#67e8f9" }}>
                {item.brand}
              </span>
            )}
          ...
          <h1 style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: "clamp(30px,4vw,50px)", letterSpacing: "-.025em", lineHeight: 1.08, margin: "0 0 12px" }}>
            {item.name}
          </h1>

          {item.description && (
            <p style={{ fontSize: 17, lineHeight: 1.6, color: "#94A3B8", margin: "0 0 22px" }}>{item.description}</p>
          )}

          <div style={{ display: "flex", alignItems: "baseline", gap: 13, marginBottom: 28 }}>
      ...
      {related.length > 0 && (
        <section style={{ marginTop: 64 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 18 }}>
            {related.map(a => (
              <RelatedCard key={a._id} item={a} onClick={() => navigate(`/accessory/${a._id}`)} />
            ))}
          </div>
        </section>
      )}

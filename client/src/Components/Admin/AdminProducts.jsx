...
export default function AdminProducts() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) { await axios.put(`${API_BASE_URL}/phones/${editingId}`, fd, { headers }); toast.success("Phone updated"); }
      else { await axios.post(`${API_BASE_URL}/phones`, fd, { headers }); toast.success("Phone created"); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await axios.delete(`${API_BASE_URL}/phones/${id}`, { headers: { Authorization: `Bearer ${getToken()}` } }); setPhones(p => p.filter(x => x._id !== id)); toast.success("Deleted"); }
      ...
      <div style={S.inner}>
        ...
        <div style={S.toolbar}>
          <div style={S.searchWrap}>
            ...
            <input style={S.searchInput} placeholder="Search phones..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        ...
        {filtered.length === 0 && !loading && <p style={S.empty}>No phones found</p>}
            ...
            <motion.div key={phone._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div style={S.cardBody}>
                ...
                <div style={S.actionRow}>
                  <button style={S.editBtn} onClick={() => handleEdit(phone)}><Edit size={13} /> Edit</button>
                  <button style={S.delBtn} onClick={() => handleDelete(phone._id)}><Trash2 size={13} /> Delete</button>
                </div>
              </div>
            </motion.div>
          ))}
          ...
          <motion.div style={S.overlay} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div style={S.modal} initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} transition={{ type: "spring", damping: 28, stiffness: 300 }}>
              <div style={S.modalHeader}>
                <h2 style={S.modalTitle}>{editingId ? "Edit phone" : "New phone"}</h2>
                <button style={S.closeBtn} onClick={resetForm}><X size={18} /></button>
              </div>
              <div style={S.formScroll}>
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <input required placeholder="Phone name *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} style={S.input} />
                  ...
                  <div style={S.row2}>
                    <input placeholder="Storage (e.g. 128 GB)" value={form.storage} onChange={e => setForm({ ...form, storage: e.target.value })} style={S.input} />
                    <input placeholder="RAM (e.g. 8 GB)" value={form.ram} onChange={e => setForm({ ...form, ram: e.target.value })} style={S.input} />
                  </div>
                  <div style={S.row2}>
                  ...
                  <textarea placeholder="Description (optional)" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} style={S.textarea} />
